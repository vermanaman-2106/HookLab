...
export default function SignupPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setInfo(
        "Check your email to confirm your account, then sign in. (Disable email confirmation in Supabase for instant access in development.)"
      );
    } finally {
      setLoading(false);
    }
  ...
  async function handleGoogle() {
    ...
    try {
      const { error: err } = await signInWithGoogle("/app");
      if (err) {
    ...
    <div className="flex min-h-dvh flex-col items-center justify-center bg-[#0b0b0f] px-4">
      <div className="w-full max-w-[400px] rounded-2xl border border-[#1f1f26] bg-[#111116] p-8 shadow-[0_24px_80px_-24px_rgba(0,0,0,0.6)]">
        ...
        <p className="mt-2 text-center text-sm text-gray-500">
          Google, or email and password — no OTP in the app.
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          {info ? (
            <p className="rounded-lg bg-orange-500/10 px-3 py-2 text-sm text-orange-200/90">
              {info}
            </p>
          ) : null}
